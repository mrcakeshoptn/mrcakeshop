...
**Project pages vs. custom domain:** if you're deploying to
`https://<your-username>.github.io/<repo-name>/` (a *project* page, no custom domain), set
`NEXT_PUBLIC_BASE_PATH` to `/<repo-name>` in the workflow's `env:` block before building. If
you're using a custom domain or a user/org page (`<username>.github.io`), leave it blank — it
...
- Set weight-based pricing, shape charges, eggless charges, and step rules per cake
- Upload and manage product images (auto-compressed for browser storage)
- Configure business details, WhatsApp number, currency, and branding
...
| Logo / favicon / hero image | Admin → Settings → Branding | none set |
